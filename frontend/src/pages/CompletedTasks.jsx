...
const CompletedTasks = () => {
        ...
        const fetchTasks = async () => {
            try {
                ...
                const response = await axios.get(
                    `http://localhost:1000/api/v2/get-complete-tasks?page=${page}`,
                    { headers }
                );
    ...
    return (
        <div className="p-4 overflow-y-auto max-h-screen" onScroll={handleScroll}>
            <h1 className="text-3xl font-bold text-center mb-4 text-gray-900">Completed Tasks</h1>
            {data.length > 0 ? (
                <Cards tasks={data} toggleInputModal={() => {}} setTasks={setData} setUpdatedTask={() => {}} />
            ) : (
                <p className="text-center text-gray-400">No completed tasks available.</p>
            )}
